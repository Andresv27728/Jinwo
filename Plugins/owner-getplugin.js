...
const handler = async (m, {conn, isROwner, usedPrefix, command, text}) => {
  ...
  if (!text) throw `**Ｏ(≧∇≦)Ｏ🌕* *🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖* Ingresa el nombre de algún archivo  🌕\n\n*—◉ 𝙴𝙹𝙴𝙼𝙿𝙻𝙾*\n*◉ ${usedPrefix + command}* info-infobot\n\n*—◉ 𝙻𝙸𝚂𝚃𝙰 𝙳𝙴 𝙿𝙻𝚄𝙶𝙸𝙽𝚂 (𝙰𝚁𝙲𝙷𝙸𝚅𝙾𝚂) 𝙴𝚇𝙸𝚂𝚃𝙴𝙽𝚃𝙴𝚂:*\n*◉* ${ar1.map((v) => ' ' + v).join`\n*◉*`}`;
  if (!ar1.includes(text)) return m.reply(`*Ｏ(≧∇≦)Ｏ🧃* *Pikachu-Bot* *Estimado dueño no se encontró un plugin (𝙰𝚁𝙲𝙷𝙸𝚅𝙾) 𝙻𝙻𝙰𝙼𝙰𝙳𝙾 "${text}", ingresa alguno existente *\n\n*==================================*\n\n*—◉ *Ｏ(≧∇≦)Ｏ🌕 *🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖* *aquí están los archivos existentes:*\n*◉* ${ar1.map((v) => ' ' + v).join`\n*◉*`}`);
  let o;
  try {
