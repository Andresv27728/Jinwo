...
async function reportError(e) {
  await m.reply(`🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖: ¡jueputa! ocurrió un error inesperado 🤬`)
  console.log(e)
}
...
case isCommand1:
  ...
  if (!await fs.existsSync(path)) {
    await conn.sendMessage(m.chat, { 
      text: `🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖: no se detectó ninguna sesión activa 🔍\n\nUsa: *${usedPrefix + command}*\nO con ID:\n*${usedPrefix + command} (ID)*` 
    }, { quoted: m })
    return
  }
  ...
  if (global.conn.user.jid !== conn.user.jid) {
    ...
    await conn.sendMessage(m.chat, { 
      text: `🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖: ¡sub-bot desconectado exitosamente! 💤` 
    }, { quoted: m })
  }
...
case isCommand2:
  if (global.conn.user.jid == conn.user.jid) {
    ...
    await conn.reply(m.chat, `🤖🌕YᗩᘜIᖇᗩ-ᗷOT-ᗰᗪ🌕🤖 se ha *desactivado temporalmente* ⚠️`, m)
    conn.ws.close()
  }  
  break
